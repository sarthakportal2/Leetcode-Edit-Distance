class Solution {
    int solve(int i , int j , String s1 , String s2 , int dp[][]){
        //new_approach
        if( i == 0 && j == 0){
        if(s1.charAt(i) == s2.charAt(j)) return 0 ;else return 1 ;}
        if( j == -1) return i+1 ; 
        if(i == -1 ) return j+1 ;  
        if(dp[i][j] != -1) return dp[i][j] ; 
        int insert = 300000000 ,replace = 30000000 ,delete = 30000000 ; 
       ...
       // if(i==0 && j==0){
          // if(s1.charAt(i)==s2.charAt(j))return 0;else return 1;}
           // if(j==-1)return i+1;
            //if(i==-1)return j+1;
            //if(dp[i][j]!=-1)return dp[i][j];
            //int insert =30000000,repl=30000000,del=30000000;
            //if(s1.charAt(i)==s2.charAt(j))return dp[i][j]=solve(i-1,j-1,s1,s2,dp);}
            //else {
              //  insert=solve(i,j-1,s1,s2,dp);
                //repl=1+solve(i-1,j-1,s1,s2,dp);
               // del=1+solve(i-1,j,s1,s2,dp);}
            //return dp[i][j]=Math.min(insert.Math.min(repl,del);
        //}
        //public int min dist(String w1,String w2){
          //  dp[][]=new int[w1.length()][w2.length()];
        //}
        //for(int []I:dp){
          //  Arrays.fill(I-1);
            //return solve(w1.length()-1,w2.length()-1,w1,w2);
